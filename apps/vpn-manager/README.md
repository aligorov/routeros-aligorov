...
2. При установке Winbox спросит два секрета:
   - `db-password` — пароль PostgreSQL (используется и приложением);
   - `admin-password` — пароль первого входа в админку.
3. Откройте UI (кнопка в Winbox или `http://<ip-роутера>:3000`), войдите
   `admin` / значение `admin-password` и **смените пароль** в профиле.
